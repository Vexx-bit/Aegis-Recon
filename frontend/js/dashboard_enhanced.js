    ...
    try {
        ...
        modalBody.innerHTML = `
            ...
            <div id="reportContent" class="report-printable">
                <!-- Report Header -->
                <div style="text-align: center; margin-bottom: 2rem; padding-bottom: 1.5rem; border-bottom: 3px solid #3b82f6;">
                    <h1 style="color: #3b82f6; margin: 0; font-size: 1.75rem; font-weight: 700;">
                        AEGIS RECON
                    </h1>
                    <p style="color: #64748b; margin: 0.25rem 0 0 0; font-size: 0.875rem; letter-spacing: 2px;">SECURITY THREAT ASSESSMENT REPORT</p>
                </div>
                
                <!-- Report Meta -->
                <table style="width: 100%; margin-bottom: 1.5rem; background: #f8fafc; border-radius: 8px;">
                    <tr>
                        <td style="padding: 1rem;">
                            <div style="font-size: 0.7rem; color: #94a3b8; text-transform: uppercase;">TARGET</div>
                            <div style="font-weight: 700; color: #0f172a;">${target}</div>
                        </td>
                        <td style="padding: 1rem; text-align: right;">
                            <div style="font-size: 0.7rem; color: #94a3b8; text-transform: uppercase;">GENERATED</div>
                            <div style="font-weight: 700; color: #0f172a;">${scanDate}</div>
                        </td>
                    </tr>
                </table>
                ...
                <!-- Footer - Using inline styles for PDF compatibility -->
                <div style="margin-top: 3rem; padding-top: 1rem; border-top: 2px solid #3b82f6; text-align: center;">
                    <p style="margin: 0; color: #475569; font-size: 0.875rem;">
                        Generated by <strong>Aegis Recon Security Platform</strong>
                    </p>
                    <p style="margin: 0.5rem 0 0 0; color: #64748b; font-size: 0.75rem;">
                        © ${new Date().getFullYear()} - Confidential Security Assessment
                    </p>
                </div>
            </div>
        `;
...
function downloadReportPDF() {
    ...
    const filename = `Aegis_Recon_Report_${target}_${dateStr}.pdf`;
    
    // Improve PDF layout by avoiding page breaks inside elements
    const style = document.createElement('style');
    style.textContent = `
        @media print {
            .report-body h4 { page-break-after: avoid; }
            .report-body li { page-break-inside: avoid; }
            .report-body p { orphans: 3; widows: 3; }
        }
    `;
    element.appendChild(style);
    
    const opt = {
        margin: [15, 15, 20, 15], // top, left, bottom, right (more bottom margin for footer)
        filename: filename,
